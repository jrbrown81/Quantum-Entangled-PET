...
// Bottom
	tree[0][0]->ReadFile("bottom_trueScatter_0-20.csv");
	tree[0][1]->ReadFile("bottom_trueScatter_80-100.csv");
	tree[0][2]->ReadFile("bottom_scatter_noPhiWindow.csv");
	tree[0][3]->ReadFile("bottom_true_80-100.csv");
// Middle
	tree[1][0]->ReadFile("middle_trueScatter_0-20.csv");
	tree[1][1]->ReadFile("middle_trueScatter_80-100.csv");
	tree[1][2]->ReadFile("middle_scatter_noPhiWindow.csv");
	tree[1][3]->ReadFile("middle_true_80-100.csv");
	...
	for(int i=0;i<2;i++) {
		...
		scatAbsScaled_h[i]=new TH1F(Form("scatAbsScaled%i_h",i),"'Real' scatter profile",nBins,0,nBins);//
...
// Create canvases
	...
	c1[1]=new TCanvas("c1_2","Raw slices (middle)");
	TLegend* leg1[3];
...
// Phi 0 and 90 only method
	...
	for(int i=0;i<2;i++){
		scatEst0_90_h[i]=new TH1F(Form("scatEst0_90_h_%i",i),"Extracted scatter profile",nBins,0,nBins);
		...
		scat90_h[i]=new TH1F(Form("scat90_h_%i",i),"Extracted scatter profile scaled for 80<|#phi|<100",nBins,0,nBins);
...
/////////////////////////////////////////
	...
	TCanvas *c2[3];
	c2[0]=new TCanvas(Form("c2_%i",0),"Scatter extraction, bottom (histo)");
	c2[1]=new TCanvas(Form("c2_%i",1),"Scatter extraction, middle (histo)");
	TLegend* leg2[3];
	...
	TCanvas *c4[3];
	TCanvas *c6[3];
	c3[0]=new TCanvas("c3_0","Scatter extraction, bottom (l)");
	c3[1]=new TCanvas("c3_1","Scatter extraction, middle (l)");
	c4[0]=new TCanvas("c4_0","Scatter extraction, bottom (l)");
	c4[1]=new TCanvas("c4_1","Scatter extraction, middle (l)");
	c6[0]=new TCanvas("c6_0","True extraction (bottom)");
	c6[1]=new TCanvas("c6_1","True extraction (middle)");
	TLegend* leg3[3];
	TLegend* leg6[3];
	TLatex *tl[4];
	for(int c=0;c<4;c++) {
	...
	for(int i=0;i<2;i++) {
		...
		leg3[i]->AddEntry(f1[i],"Fit to extracted scatter");
		leg3[i]->SetTextSize(legTextSize);
		leg3[i]->Draw();

		c6[i]->cd();
		true90_h[i]->SetLineColor(2);
		true90_h[i]->GetXaxis()->SetTitleSize(labelTextSize);
		true90_h[i]->GetYaxis()->SetTitleSize(labelTextSize);
		true90_h[i]->GetXaxis()->SetLabelSize(labelTextSize);
		true90_h[i]->GetYaxis()->SetLabelSize(labelTextSize);
		true90_h[i]->GetYaxis()->SetTitleOffset(titleOffset);
		true90_h[i]->DrawCopy("l hist");
		trueAbs_h[i]->DrawCopy("l hist same");
		
		leg6[i] = new TLegend(leftMargin,0.75,leftMargin+0.4,1-topMargin);
		leg6[i]->AddEntry(true90_h[i],true90_h[i]->GetTitle());
		leg6[i]->AddEntry(trueAbs_h[i],trueAbs_h[i]->GetTitle());
	...
	TCanvas* scaleScatTest_c=new TCanvas("scaleScatTest_c","Scaled profile comparison for scatter extraction");
	TH1F* phi0ScaledScat_h=(TH1F*)phi_h[0][0]->Clone("phi0ScaledScat_h");
	TH1F* phi90ScaledScat_h=(TH1F*)phi_h[0][4]->Clone("phi90ScaledScat_h");
	phi0ScaledScat_h->Scale(c_t[0][4]/K[0]);
	phi90ScaledScat_h->Scale(c_t[0][0]/K[0]);
	phi0ScaledScat_h->Draw("hist ");
	phi90ScaledScat_h->Draw("hist  same");
	
	TCanvas* scaleTrueTest_c=new TCanvas("scaleTrueTest_c","Scaled profile comparison for true extraction");
	TH1F* phi0ScaledTrue_h=(TH1F*)phi_h[0][0]->Clone("phi0ScaledTrue_h");
	TH1F* phi90ScaledTrue_h=(TH1F*)phi_h[0][4]->Clone("phi90ScaledTrue_h");
	phi0ScaledTrue_h->Scale(c_t[0][4]*c_s[0][4]/K[0]);
	phi90ScaledTrue_h->Scale(c_t[0][4]*c_s[0][0]/K[0]);
	phi90ScaledTrue_h->Draw("hist ");
	phi0ScaledTrue_h->Draw("hist  same");
