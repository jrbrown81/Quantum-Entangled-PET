   ...
   for(i=0;i<n_bin;i++){
     ...
     for(j=0;j<n_ang;j++){
       ...
       /* else{
         ...
         int lastSegP;
         k=0;
         for(int s=0;s<=nSEG;s++){   //TO DO: aÃ±adir aqui MRDiff!!!!
           if(s==0){
 ...
 // ------------------------------ end SSRB part ----------------------------------- //
   ...
   sino_FFT = new double*[n_bin];   //n_bin=R, en primera versiÃ³n no se usa Rmod, se chequea dentro del bucle
   for(i=0;i<n_bin;i++){
 ...
 for(j=0;j<y;j++){
 for(i=0;i<x;i++){
  // double radius = sqrt((x-150)*(x-150)+(y-150)*(y-150));
  //   cout << radius << endl;
