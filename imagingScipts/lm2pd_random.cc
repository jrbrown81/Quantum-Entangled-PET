...
int main(int argc, char *argv[])
{
  if (argc == 1)
  {
    ...
    "     -b Type of coincidence: -1 = true + random (by default), 0 = true, 1 = random, <random> \n"
  ...
  cout << "lm2pd::Parameters: planes = " << n_planes << "; bins = " << n_bin
   << "; ang_views = " << n_ang << endl;
  cout << "lm2pd::Dimensions: Transaxial FOV = " << d_FOV << "; Axial FOV = "
   << d_axial << " ; Transaxial_pix (mm) = " << d_FOV / (n_bin - 1)
   <<"; Plane_width (mm) = " << d_axial / (n_planes - 1) << endl;
  
  if (coincidenceType == -1)
   cout << "True, scatter and random coincidences are considered" << endl;
  
  if (coincidenceType == 0)
   cout << "Only true coincidences are considered" << endl;
  
  if (coincidenceType == 1)
   cout << "Only scatter coincidences are considered" << endl;
 
  if (coincidenceType == 2)
   cout << "Only random coincidences are considered" << endl;

  if (coincidenceType == 3)
   cout << "Only true and random coincidences are considered" << endl;
  ...
  // 4 protections to avoid incoherent values for the upper and lower endpoints
  // of the windows (theta, |deltaPhi| and energy lost)
  ...
  {
    if (verbos == 0 && nbTotCoincidences % 10000000 == 0)
    {
    ...
    if (verbos >= 3)
    {
      cout << nbTotCoincidences << " " << theLOR.p1.windowID << " LoR_p1: " 
       << theLOR.p1.theta << " " << theLOR.p1.X_a << " " << theLOR.p1.Y_a
       ...
       << " " << theLOR.p2.E_b << " " << theLOR.p2.coincidence
  ...
  cout << "Accepted coincidences for the sinogram: " << nbCoinciVal << endl;
  cout << "True coincidences: " << nbTrueTot << endl;
  cout << "Scatter coincidences: " << nbScatterTot << endl;
  cout << "Random coincidences: " << nbRandomTot << endl;
...
void Load_sino3D(lor inpLOR)
{
  ...
  // To treat only true or only scatter, or only random coincidences
  // OR only true + random coincidences
  if ((coincidenceType != -1 && coincidenceType != 3
   && coincidenceType != inpLOR.p2.coincidence) 
   || (coincidenceType == 3 && inpLOR.p2.coincidence == 1)) return;
  ...
  // 1st method: apply a |deltaPhi| window
  ...
  if (inpLOR.p2.coincidence == 0) nbTrueTot++;
  if (inpLOR.p2.coincidence == 1) nbScatterTot++;
  if (inpLOR.p2.coincidence == 2) nbRandomTot++;
}
...
lor ReadData()
{
  ...
  if (typeINfile == 0)
  {
    ...
    fscanf(theINfile,
     "%d%*c %lf%*c %lf%*c %lf%*c %lf%*c %lf%*c %lf%*c %lf%*c %lf%*c %lf%*c %lf%*c %lf%*c %lf%*c %lf%*c %lf%*c %lf%*c %lf%*c %lf%*c %lf%*c %d%*c %lf",
     &window, &theta1, &X1_a, &Y1_a, &Z1_a, &E1_a, &X1_b, &Y1_b, &Z1_b, &E1_b,
     &theta2, &X2_a, &Y2_a, &Z2_a, &E2_a, &X2_b, &Y2_b, &Z2_b, &E2_b,
     &type, &deltaPhi);

    lorOUT.p1.windowID = window;
    ...
    lorOUT.p1.Z_b = Z1_b;
    lorOUT.p1.E_b = E1_b;
    lorOUT.p1.coincidence = 0;
    lorOUT.p1.deltaPhi = 0.;
    ...
    // Type of coincidence = true or random
    lorOUT.p2.coincidence = type;
